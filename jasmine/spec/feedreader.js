    ...
    */
    ...
    describe('The menu', function() {
        ...
        it('Menu is hidden by default', function() {
          ...
          */
        it('Menu Toggling is working', function() {
    ...
    describe('Initial Entries', function() {
        ...
        it('Load Feed is working', function() {
            expect($('.entry-link').length).toBeGreaterThan(0);
        });
    });
    /* Test suite named "New Feed Selection"*/
    describe('New Feed Selection', function() {
        /* A test that ensures that content changes in new feed selection.
         */
        var contentURL = '';
        beforeEach(function(done) {
            loadFeed(0,function(){
                contentURL = $('.entry-link').eq(0).prop('href');
                loadFeed(1, done);
            });
        });
        it('Content is changing on Load Feed', function() {
            expect(contentURL).not.toBe($('.entry-link').eq(0).prop('href'));
        });
    });
}());
